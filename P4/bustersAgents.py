...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            max_ghost = None
            max_prob = -1
            for ghost, prob in dist.items():
                if prob > max_prob:
                    max_ghost = ghost
                    max_prob = prob
            maxBelief.append(max_ghost)

        minVal = float('inf')
        minAct = None
        for action in legal:
            successor_position = Actions.getSuccessor(pacmanPosition, action)
            maxBeliefDistances = [self.distancer.getDistance(successor_position, ghost) for ghost in maxBelief]
            val = min(maxBeliefDistances)
            if val < minVal:
                minVal = val
                minAct = action
        return minAct
